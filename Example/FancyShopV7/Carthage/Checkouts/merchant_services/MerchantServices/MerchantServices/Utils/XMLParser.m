...
+ (XMLParser *)sharedInstance {
    ...
    static XMLParser *sharedInstance;
    dispatch_once(&once, ^{
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    XMLParser *copy = [[[self class] allocWithZone:zone] init];
...
+ (NSDictionary *)dictionaryWithXMLParser:(NSXMLParser *)parser {
    ...
    return [[[XMLParser sharedInstance] copy] dictionaryWithParser:parser];
}

+ (NSDictionary *)dictionaryWithXMLData:(NSData *)data {
    
    return [[[XMLParser sharedInstance] copy] dictionaryWithData:data];
}

+ (NSDictionary *)dictionaryWithXMLString:(NSString *)string {
    
    return [[[XMLParser sharedInstance] copy] dictionaryWithString:string];
}

+ (NSDictionary *)dictionaryWithXMLFile:(NSString *)path {
    
    return [[[XMLParser sharedInstance] copy] dictionaryWithFile:path];
}
...
- (NSString *)innerXML {
    ...
    for (NSString *key in childNodes) {
        ...
        [nodes addObject:[XMLParser XMLStringForNode:childNodes[key] withNodeName:key]];
    }
...
- (NSString *)XMLString {
    ...
    if ([self count] == 1 && ![self nodeName]) {
        ...
        return [XMLParser XMLStringForNode:self withNodeName:[self nodeName] ?: @"root"];
    }
}
